  ...
  return (<>
    <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-sheet" style={{ maxHeight: '88vh' }}>
        ...
        <div className="modal-header"><div className="modal-title">{t.pinStatusTitle}</div><button className="modal-close" onClick={onClose}><IconX size={14} /></button></div>
        <div className="modal-body">
          <div className="step-label">{t.enterPin}</div>
          ...
          {result === 'invalid' && <div style={{ textAlign: 'center', padding: '28px 0', color: 'var(--text3)' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: '#ff6b6b', marginBottom: 5 }}>{t.invalidPincode}</div>
            <div style={{ fontSize: 12, lineHeight: 1.6 }}>{pin} {t.notAValidPincode}</div>
          </div>}
          ...
          {result === 'nodata' && pinInfo && <div style={{ animation: 'successIn .4s var(--spring)' }}>
            <div style={{ background: 'var(--card)', border: '1px solid var(--border2)', borderRadius: 16, padding: '18px', marginBottom: 14, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)', padding: '8px 12px', background: 'var(--card2)', borderRadius: 8, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                <IconCloudDrizzle size={14} color="var(--text3)" /> {t.noRainData}
              </div>
            </div>
          </div>}
          ...
          {result && result !== 'invalid' && result !== 'nodata' && (() => {
            ...
            return (<div style={{ animation: 'successIn .4s var(--spring)' }}>
              <div style={{ background: `${col}10`, border: `1px solid ${col}28`, borderRadius: 16, padding: '18px', marginBottom: 14, textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--mono)', fontSize: 24, fontWeight: 800, color: col, marginBottom: 4 }}>{ghost ? rawAvg.toFixed(1) : effAvg.toFixed(1)} <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--text3)' }}>{t.mmhr}</span></div>
                {ghost ? <div style={{ fontSize: 11, color: '#7a8899', marginBottom: 6 }}>{t.fadedBadge} · {t.peakMmhr} {rawAvg.toFixed(1)}</div> : <div className={`rc-badge badge-${level}`} style={{ display: 'inline-flex', marginBottom: 8 }}>{getIntensityLabel(effAvg).toUpperCase()}</div>}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7, marginBottom: 14 }}>
                {[
                  { val: ghost ? rawAvg.toFixed(1) : effAvg.toFixed(1), lbl: ghost ? t.peakMmhr : t.nowMmhr, col },
                  { val: r.count, lbl: t.reportCount, col: 'var(--accent)' },
                  { val: fmtTime(r.lastUpdated, { sAgo: t.sAgo, mAgo: t.mAgo, hAgo: t.hAgo }), lbl: t.lastReport, col: 'var(--text2)' },
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <button onClick={() => waShare(buildShareText(r.pin, r.place, r.district, rawAvg, r.count))} style={{ ...sbStyle, background: 'rgba(37,211,102,0.07)', border: '1px solid rgba(37,211,102,0.2)', color: '#25d366' }}>
                  <IconWhatsApp size={16} color="#25d366" />{t.whatsapp}
                </button>
                <button onClick={() => setShareData({ title: r.pin, text: buildShareText(r.pin, r.place, r.district, rawAvg, r.count) })} style={{ ...sbStyle, background: 'rgba(0,212,255,0.07)', border: '1px solid var(--border3)', color: 'var(--cyan)' }}>
                  <IconShare size={16} color="var(--cyan)" />{t.share}
                </button>
              </div>
            </div>);
          })()}
