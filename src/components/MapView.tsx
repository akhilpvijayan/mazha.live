...
function markerRadius(zoom: number, intensity: number, selected: boolean): number {
  // Base size scales with zoom so markers stay proportional to geography
  const base = zoom <= 6  ? 4
             : zoom === 7 ? 5
             : zoom === 8 ? 6
             : zoom === 9 ? 7
             ...
             : zoom === 11 ? 9
             : 10;
  // Tiny intensity bonus — max 3px extra so clusters don't grow huge
    ...
    <div className="modal-backdrop" onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div className="modal-sheet" style={{maxHeight:'88vh'}}>
        ...
        <div style={{display:'flex',gap:6,padding:'10px 16px',overflowX:'auto',borderBottom:'1px solid var(--border)',flexShrink:0}}>
          {CHIPS.map(c=>{const col=c.id==='ghost'?'#7a8899':c.id!=='all'?BADGE_COLORS[c.id as Level]:'var(--text2)';const active=filter===c.id;
...
function InsightsTab({reports,now}:{reports:RainReport[];now:number}) {
  ...
  return<div>
    ...
    {districts.length>0&&<div className="insights-section">
      ...
      {districts.map(d=>{const col=d.avg>80?'#ff3b3b':d.avg>50?'#ff7a00':d.avg>20?'#a855f7':'#00d4ff';return(
    ...
    <div className="insights-section">
      ...
      {active.filter(r=>currentAvgIntensity(r,now)>50).length===0
        ...
        :active.filter(r=>currentAvgIntensity(r,now)>50).map(r=>{const eff=currentAvgIntensity(r,now);const col=eff>80?'#ff3b3b':'#ff7a00';return(
...
function EngagementPanel({ reports, now }: { reports: RainReport[]; now: number }) {
  ...
  // uptime since epoch start (fake engagement metric)
      ...
      <div className="modal-sheet" style={{maxHeight:'80vh'}}>
        ...
        <div className="modal-body">
          ...
          <div style={{textAlign:'center',fontSize:10,color:'var(--text3)',marginTop:10}}>
            Opens Google Pay / any UPI app · UPI ID: akhilakhil@okaxis
          </div>
        </div>
      </div>
    </div>
...
export default function MapView() {
  ...
  const [activeNav,setActiveNav]         = useState<'radar'|'activity'|'insights'>('radar');
  const [lastUpdated,setLastUpdated]     = useState(0);
  const mapRef = useRef<any>(null);

  // Filter out expired reports (>48h) and compute live set
  ...
  // Load from Supabase on mount, fallback to memory
  useEffect(() => {
  ...
  // Tick: update `now` every 30s so decay animates
  useEffect(() => {
  ...
  const districtStyle = useCallback(() => ({
    color: theme==='dark'?'rgba(0,212,255,0.14)':'rgba(0,100,180,0.17)',
    weight: 0.7, fillOpacity: 0, opacity: 1, dashArray: '3 5',
  }), [theme]);
  ...
  return (
    <>
      <div className="map-shell">
        ...
        {heavyReport && (
          <div className="map-banner">
            <IconAlertTriangle size={13} color="var(--cyan)"/>
            {t.intenseBanner} <span className="banner-highlight">{heavyReport.district}</span>
            &nbsp;·&nbsp;<span className="banner-count">{reports.length * 12} {t.reportsIn}</span>
          </div>
        )}

        <div className="map-tools">
          <button className="map-tool-btn" title="Reset" onClick={()=>mapRef.current?.flyTo([10.8505,76.2711],7,{duration:1})}><IconMap size={16}/></button>
          <button className={`map-tool-btn${showHeat?' on':''}`} title="Heatmap" onClick={()=>setShowHeat(p=>!p)}><IconFire size={16}/></button>
          {canInstall && <button className="map-tool-btn" title="Install App" onClick={install}><IconInstall size={16}/></button>}
        </div>
        ...
        {/* Floating PIN search icon - mobile only */}
      ...
      <nav className="bottom-nav">
        {([
          {id:'radar'as const,    label:t.radar,    Icon:IconCloudRain},
          {id:'activity'as const, label:t.activity, Icon:IconActivity},
          {id:'insights'as const, label:t.insights, Icon:IconBarChart},
        ]).map(n=>(
          <button key={n.id} className={`nav-item${activeNav===n.id?' active':''}`}
            onClick={()=>{
              if(n.id==='radar'){setActiveNav('radar');setMobileSheet(false);}
              else{
                if(activeNav===n.id&&showMobileSheet){setMobileSheet(false);setActiveNav('radar');}
                else{setActiveNav(n.id);setMobileSheet(true);}
              }
            }}>
            <span className="nav-icon-wrap"><n.Icon size={20}/></span>{n.label}
          </button>))}
        {/* Districts button in bottom nav */}
        <button className={`nav-item${showDistShare?' active':''}`} onClick={()=>setShowDist(true)}>
          <span className="nav-icon-wrap"><IconShare size={20}/></span>Districts
        </button>
      </nav>

      {showMobileSheet && activeNav !== 'radar' && (
